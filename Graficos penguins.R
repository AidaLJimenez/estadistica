...
color=c("darkseagreen1","lightpink")
...
# 1.- Creación de un vector de color
color=c("hotpink", "gold", "cyan3")

# 2.- Creación del gráfico
GB1<-ggplot(BD1, aes(x=año))+
  geom_bar(colour= "green", fill=color)+
...
GB2<-ggplot(BD1, aes(x=año))+
  geom_bar(colour= "lightcoral", fill=color)+
  ggtitle("Gráfico de Barras")+
  xlab("Año")+
  ylab("Frecuencias")+
  coord_flip()+
...
GD2<-ggplot(BD1, aes(x=largo_pico_mm, y=grosor_pico_mm))+
  ...
  theme_light()
   ...
   GD2<-ggplot(BD1, aes(x=largo_pico_mm, y=grosor_pico_mm))+
     geom_point(aes(color=especie))+
     scale_color_manual(values=c("hotpink" , "gold" , "cyan2"))+
     ggtitle("Gráfico de dispersión")+
     xlab("largo del pico (mm)")+
     ylab("grosor del pico (mm)")+
     theme_light()
